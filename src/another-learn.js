...
import FULLNAME from './another-learn-fullname';
...
var MENU_MAP = {
  ...
  "Cholecalciferol": "cholecalciferol.sdf",
...
function get_type(name) {
  ...
  if (PAR_MAP[sname] == undefined) return "default";
    ...
    if (draw_atoms && elements[i][1] == 0) {
      ...
      if (
        ...
        if (letters_shading) {
          ...
          if (IS_SAFARI) point3d(ctx, x, y, z, (0.5+col1/512)*atom_radius);
      ...
      if (bonds_autowidth) ctx.lineWidth = 0.1 + col1 / 120.0;
...
function Molecule(pars) {
  ...
  this.render = function() {
    ...
    render_molecule(
      ...
      this.draw_bonds,
      this.bonds_color,
      this.bonds_autowidth,
      this.bonds_gradient,
      this.atom_colors,
      this.atom_radius,
      this.flat_shading,
      this.sphere_shading,
      this.letters_shading,
      this.zback,
      this.zrange
    );
  }
  ...
  this.toggle_autorotation = function(axis, what) {
    ...
    if (this.timeout_id >= 0) clearTimeout(this.timeout_id);
    var auto_on = this.autorotation[0] ||
    this.autorotation[1] ||
    this.autorotation[2];
    if (auto_on) {
  ...
  for (var r = 0; r < MENDELEEV.length; ++r) {
    for(var c = 0; c < MENDELEEV[r].length; ++c) {
      ...
      if(element) {
        ...
        var div = $(
          "<div class='mm' id='m_" + element +
          "' title='" + FULLNAME[element] +
...
function get_parameter(url) {
  ...
  if (chunks.length == 2) return unescape(chunks[1].toLowerCase());
  return "";
}
...
$(document).ready(function(){
  ...
  try {
    add_molecule = add_molecule_flash;
  } catch(e) {

  }
  //if(has_webgl()) add_molecule = add_molecule_webgl;

  var parameter = get_parameter(window.location.href);
  if (MENU_MAP_LO[parameter] != undefined) {
    add_molecule(create_permalink(capitalize(parameter)), MENU_MAP_LO[parameter]);
  } else {
    for(var i = 0; i < INIT_NAMES.length; ++i) {
      var label = capitalize(INIT_NAMES[i].substr(0, INIT_NAMES[i].length - 4));
      add_molecule(label, INIT_NAMES[i]);
    }
  }

  create_molecule_menu("mitems", add_molecule);
  create_periodic_table("colors");

  $(".drag").draggable();

  $('textarea#pdbtext').click(function() {
    $(this).focus();
    $(this).select();
  });

  $("#b_import").click(function() {
    var text = $('textarea#pdbtext').val();
    add_molecule("Imported molecule", "", text);
  });

  $("#b_controls").click(function() {
    $(".touch").toggle();
    TAP_ON = !TAP_ON;
  });
