...
function WebGLRenderer(parameters) {
  ...
  var _this = this,
	...
	// vr
  var vr = (typeof navigator !== 'undefined' && 'xr' in navigator)
  ...
  : new WebVRManager(_this);
	...
	this.renderBufferDirect = function (camera, fog, geometry, material, object, group) {
      ...
      _currentGeometryProgram.wireframe !== (material.wireframe === true)
		...
		if (updateBuffers) {
			...
			if (index !== null) _gl.bindBuffer(_gl.ELEMENT_ARRAY_BUFFER, attribute.buffer);
		}
	...
	this.render = function (scene, camera) {
		...
		if (vr.enabled && vr.isPresenting()) camera = vr.getCamera(camera);
		...
		if (vr.enabled && multiview.isAvailable()) multiview.attachCamera(camera);
		...
		if (vr.enabled) {
			if (multiview.isAvailable()) multiview.detachCamera(camera);
			vr.submitFrame();
		}
	...
	this.readRenderTargetPixels = function (renderTarget, x, y, width, height, buffer, activeCubeFaceIndex) {
		...
		if (framebuffer) {
			...
			try {
				...
				if (
          textureType !== UnsignedByteType && utils.convert(textureType) !== _gl.getParameter(_gl.IMPLEMENTATION_COLOR_READ_TYPE) && // IE11, Edge and Chrome Mac < 52 (#9513)
