...
var SkeletonUtils = {
	retarget: function () {
		...
		return function (target, source, options) {
			...
			if (options.preserveMatrix) target.updateMatrixWorld(true);  // restore matrix
		};
	}(),
	...
	retargetClip: function (target, source, clip, options) {
		...
		if (! source.isObject3D) source = this.getHelperFromSkeleton(source);
	...
	renameBones: function (skeleton, names) {
		...
		for (var i = 0; i < bones.length; ++i) {
			var bone = bones[i];
			if (names[bone.name]) bone.name = names[bone.name];
		}
		return this;
	},
	...
	getNearestBone: function (bone, names) {
		while (bone.isBone) {
			if (names.indexOf(bone.name) !== - 1) return bone;
			bone = bone.parent;
		}
	},
	...
	findBoneTrackData: function (name, tracks) {
		...
		for (var i = 0; i < tracks.length; ++i) {
			...
			if (trackData && name === trackData[1]) result[trackData[2]] = i;
		}
