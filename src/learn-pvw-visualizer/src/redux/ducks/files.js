...
const externalActions = {network: netActions};
...
export const initialState = {listings: {}, activePath: '.'};
...
export default function reducer(state = initialState, action) {
  switch (action.type) {
    ...
    case RESET_DIRECTORY_STORE: return {};
...
export function fetchServerDirectory(pathToList) {
  return (dispatch) => {
    ...
    .then(
      ...
      (err) => dispatch(externalActions.network.error(netRequest.id, err))
    );
    return netRequest;
  };
}