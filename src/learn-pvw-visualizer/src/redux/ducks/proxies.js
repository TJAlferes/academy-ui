...
export default function reducer(state = initialState, action) {
  switch (action.type) {
    ...
    case PIPELINE_STORE: {
      ...
      pipeline.sources.forEach(proxy => sourceToRepresentation[proxy.id] = proxy.rep);
    ...
    case UPDATE_PROPERTIES: {
      const changeMap = {};
      action.properties.forEach(prop => changeMap[propertyKey(prop)] = prop);
...
export function fetchAvailableProxies() {
  return (dispatch) => {
    ...
    .then(
      ...
      (err) => dispatch(netActions.error(netFilterRequest.id, err))
    );
    return netRequest;
  };
}
