...
export default function reducer(state = initialState, action) {
  switch (action.type) {
    ...
    case REMOTE_INTERACTIVE_QUALITY: {
      return Object.assign({}, state, {interactiveQuality: action.quality});
    }
    case REMOTE_INTERACTIVE_RATIO: {
      return Object.assign({}, state, {interactiveRatio: action.ratio});
    }
    case EVENT_THROTTLING_TIME: {
      return Object.assign({}, state, {throttleTime: action.time});
    }
    case SERVER_MAX_FPS: {
      return Object.assign({}, state, {serverMaxFPS: action.maxFPS});
    }
...
export function resetCamera() {
  ...
  return (dispatch) => {
    ...
    .then(
      (ok) => dispatch(netActions.success(netRequest.id, ok)),
      (err) => dispatch(netActions.error(netRequest.id, err))
    );
    return netRequest;
  };
}
